...
try:
    ...
    logging.exception("Failed to load Metrics dependency, prometheus metrics not available")
...
class PrometheusMetrics(BaseHTTPRequestHandler):
    ...
    def _get_metrics(self):
        try:
            ...
            logging.exception("PROMETHEUS: error while gathering metrics")
            stats = {}
...
def _start_server(m2ee, port):
    httpd = HTTPServer(('', port), PrometheusMetrics)
    httpd.m2ee = m2ee
    httpd.serve_forever()

def run(m2ee, port=8080):
    if os.environ.get("METRICS_SERVICE_ENABLED", "false").lower() != "true":
        return

    logging.info("Starting prometheus metrics service at port %d", port)
    daemon = threading.Thread(name='prometheus_metrics',
